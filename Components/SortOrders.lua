...
Bagshui:AddComponent(function()
  ...
  function SortOrders:Init()
    ...
    -- Item properties.
    self:AddValidItemFields(BS_ITEM_PROPERTIES_ALLOWED_IN_SORT_ORDERS, BS_ITEM_SKELETON)
    ...
    Bagshui.prototypes.Menus:AddAutoSplitMenu(BS_AUTO_SPLIT_MENU_TYPE.SORT_ORDERS, {
      ...
      tooltipTextFunc = function(id, originalTooltipText)
        ...
        for _, sort in pairs(self.list[id].fields) do
          ...
          local sortFieldFriendly = self.sortFields[sortField] and self.sortFields[sortField].friendlyName
            or (sort.field .. (sort.lookup and ("." .. sort.lookup) or ""))
          tooltipText = (tooltipText and tooltipText .. BS_NEWLINE or "") .. sortFieldFriendly
    ...
    Bagshui.prototypes.Menus:AddAutoSplitMenu(BS_AUTO_SPLIT_MENU_TYPE.SORT_FIELDS, {
  ...
  function SortOrders:AddValidItemFields(validFieldList, objectTemplate, lookupObjectName, lookupProperty, lookupTable)
    assert(
      not lookupObjectName or (lookupObjectName and lookupProperty and lookupTable),
      "SortOrders:AddValidItemFields(): All lookup parameters are required when one is provided"
    )
    ...
    for index = table.getn(sortOrderInfo.fields), 1, -1 do
      ...
      if not self.sortFields[fieldId] then
        if removeIfInvalid then
          ...
          assert(
            false,
            baseError .. self:GetFieldIdentifier(sort.field, sort.lookup, ".") .. " is not a valid sort field"
          )
        end
      end
  ...
  local function SortGroup_GetReversedByWordsPropertyString(item, propertyValue, propertyName)
    ...
    if propertyName == "name" and string.len(item.suffixName or "") > 0 then
      return BsUtil.ReverseStringByWords(item.baseName) .. " " .. BsUtil.ReverseStringByWords(item.suffixName)
    end
  ...
  function SortOrders:SortGroup(groupItems, groupConfig, defaultSortOrderId)
    ...
    local sortOrderId = (defaultSortOrderId and self.list[defaultSortOrderId]) and defaultSortOrderId
      or BS_DEFAULT_SORT_ORDER_ID
