...
Bagshui:AddComponent(function()
  ...
  function Categories:Init()
    ...
    self.idListSortFunctions.sequence = function(idA, idB)
      return (self._idListSortFunctions_objectList[idA].sequence or BS_DEFAULT_CATEGORY_SEQUENCE)
        < (self._idListSortFunctions_objectList[idB].sequence or BS_DEFAULT_CATEGORY_SEQUENCE)
    end
    ...
    Bagshui.prototypes.Menus:AddAutoSplitMenu(BS_AUTO_SPLIT_MENU_TYPE.CATEGORIES, {
  ...
  function Categories:OnEvent(event, arg1)
    ...
    if event == "BAGSHUI_CATEGORY_UPDATE" then
      ...
      for categoryId, categoryInfo in pairs(self.list) do
        if self.recursedCategories[categoryId] and categoryId ~= self.lastUpdatedCategoryId then
  ...
  function Categories:DoPostSaveOperations(categoryId, category)
    ...
    if type(category.list) == "table" and table.getn(category.list) > 0 then
      table.sort(category.list, function(itmA, itmB)
  ...
  function Categories:FinalizeRule(categoryId, category, ruleIdSuffix, recompile, recursed)
    ...
    if self.objectManager and self.objectManager.editors then
      for _, editor in ipairs(self.objectManager.editors) do
        if editor.objectId == categoryId and editor.uiFrame:IsVisible() and editor:IsDirty() then
  ...
  function Categories:MatchCategory(categoryId, item, character, ruleSession, recursed)
    ...
    if self.errors[categoryId] and (self.errorTimestamps[categoryId] or 0) >= category.dateModified then
  ...
  function Categories:FormatErrorMessage(errorMessage, categoryId, categoryName)
    return string.format(
      ...
      tostring(
        categoryName
          or (self.list[categoryId] and self.list[categoryId].name)
          or string.format(L.Prefix_New, L.Category)
      ),
  ...
  function Categories:GetErrors(filter)
    ...
    for _, categoryId in ipairs(self.sortedIdLists.name) do
      if self.errors[categoryId] and (type(filter) ~= "table" or filter[categoryId]) then
  ...
  function Categories:GetDirectCategoryAssignmentsForItem(itemId, categoryIdListOutput, limitToCategoryIds)
    assert(
      type(categoryIdListOutput) == "table",
      "Categories:GetDirectCategoryAssignmentsForItem(): categoryIdListOutput is required and must be the table to fill."
    )
    ...
    for categoryId, categoryInfo in pairs(self.list) do
      if categoryInfo.classes ~= nil then
        for class, classInfo in pairs(categoryInfo.classes) do
          self:ItemInDirectAssignmentList(classInfo.list, categoryId, itemId, categoryIdListOutput, limitToCategoryIds)
        end
      else
        self:ItemInDirectAssignmentList(categoryInfo.list, categoryId, itemId, categoryIdListOutput, limitToCategoryIds)
      end
    end
  end
  ...
  ---@return boolean?
  function Categories:ItemInDirectAssignmentList(
    list,
    categoryId,
    itemId,
    ...
    if not list then
      ...
      if categoryInfo.classes then
        if not class or not categoryInfo.classes[class] then
          return
        end
  ...
  function Categories:ToggleItemCategoryAssignment(categoryId, itemId, class)
    ...
    if categoryInfo.classes ~= nil then
      ...
      if categoryInfo.classes[class] and categoryInfo.classes[class].list then
        list = categoryInfo.classes[class].list
      end
    end
