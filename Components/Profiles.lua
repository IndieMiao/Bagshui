...
Bagshui:AddComponent(function()
  ...
  function Profiles:Init()
    ...
    Bagshui.prototypes.Menus:AddAutoSplitMenu(BS_AUTO_SPLIT_MENU_TYPE.PROFILES, {
  ...
  function Profiles:HandleDependenciesForImportExport(structure, export, dependencyMap)
          ...
          for i = 1, table.getn(group.categories) do
            if export then
              if not BsCategories:IsBuiltIn(group.categories[i]) then
                table.insert(export.dependencies[BsCategories], group.categories[i])
              end
    ...
    if export then
      if not BsSortOrders:IsBuiltIn(structure.defaultSortOrder) then
        table.insert(export.dependencies[BsSortOrders], structure.defaultSortOrder)
      end
    elseif dependencyMap then
        ...
        if group.sortOrder then
          if export then
            if not BsSortOrders:IsBuiltIn(group.sortOrder) then
              table.insert(export.dependencies[BsSortOrders], group.sortOrder)
            end
          elseif dependencyMap then
    ...
    for inventoryType, inventoryConfig in pairs(Bagshui.config.Inventory) do
      if inventoryConfig.dockTo then
        ...
        if not self.list[profileId].structure.docked[inventoryType].layout then
          ...
          BsUtil.TableCopy(
            BS_NEW_PROFILE_TEMPLATE.structure.primary.layout,
            self.list[profileId].structure.docked[inventoryType].layout
          )
        end
      end
  ...
  function Profiles:GetUses(objectId, objectMetadata, compact)
    ...
    if table.getn(_getUses_defaultProfiles) > 0 then
      if compact then
        ...
        self:AddUse(
          objectMetadata,
          "uses",
          "• " .. string.format(L.Symbol_Colon, L.Default) .. " " .. table.concat(_getUses_defaultProfiles, ", ")
        )
      else
        -- Default:
        -- • Structure
    ...
    for characterId, characterData in pairs(BsCharacterData.list) do
      ...
      for _, inventoryType in ipairs(BS_INVENTORY_TYPE_UI_ORDER) do
        ...
        if not Bagshui.components[inventoryType].dockTo then
          ...
          local inventoryName = L[inventoryType]
            .. (Bagshui.dockedInventories[inventoryType] and ("/" .. L[Bagshui.dockedInventories[inventoryType]]) or "")
          ...
          local inventoryData =
            characterData[Bagshui.components[Bagshui.components[inventoryType].dockTo or inventoryType].inventoryTypeSavedVars]
          ...
          for _, profileType in ipairs(BS_PROFILE_ORDER) do
            ...
            if
              inventoryData
              and inventoryData.settings
              and inventoryData.settings["profile" .. profileType] == objectId
            then
          ...
          if table.getn(_getUses_activeProfiles) > 0 then
            ...
            -- Bags (Struct, Dsgn)
            table.insert(
              _getUses_inventoryTypes,
              inventoryName .. " (" .. table.concat(_getUses_activeProfiles, ", ") .. ")"
            )
          end
        end
      end
    ...
    if not compact and table.getn(objectMetadata.usesLeft) > 0 then
      self:AddUse(
        objectMetadata,
        "uses",
        BS_FONT_COLOR.BAGSHUI .. L.Object_ProfileUses .. FONT_COLOR_CODE_CLOSE,
        nil,
        1
      )
    end
  ...
  function Profiles:GetProfilesUsingObject(objectId, objectMetadata, objectClass)
    ...
    for _, profileId in ipairs(self.sortedIdLists.name) do
      ...
      local profileName = (profileInUse and HIGHLIGHT_FONT_COLOR_CODE or GRAY_FONT_COLOR_CODE)
        .. (self:GetName(profileId, false, false) or tostring(profileId))
      ...
      if usedInProfile and table.getn(_getProfilesUsingObject_tempMetadata.usesLeft) > 0 then
        ...
        self:AddUse(
          objectMetadata,
          "usesOfProfiles",
          _getProfilesUsingObject_tempMetadata.usesLeft,
          _getProfilesUsingObject_tempMetadata.usesRight
        )
      end
    end
    ...
    if table.getn(objectMetadata.usesLeft) > 0 then
      self:AddUse(
        objectMetadata,
        "uses",
        BS_FONT_COLOR.BAGSHUI .. L.Object_UsedInProfiles .. FONT_COLOR_CODE_CLOSE,
        nil,
        1
      )
    end
      ...
      for rowNum, row in ipairs(structure.layout) do
        for groupNum, group in ipairs(row) do
          if group.sortOrder == objectId then
            table.insert(
              _getObjectUsesInStructure_details,
              group.name .. GRAY_FONT_COLOR_CODE .. " [" .. rowNum .. ":" .. groupNum .. "]" .. FONT_COLOR_CODE_CLOSE
            )
          end
        end
      end
