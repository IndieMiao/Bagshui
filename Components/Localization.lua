...
Bagshui:LoadComponent(function()
    ...
    __index = function(localeTable, str)
      ...
      if
        ...
        and (
          ...
          and not (Localization.activeLocale[tostring(str)] or Localization.activeLocale[string.lower(tostring(str))])
        )
      then
      ...
      return
        ...
        (Localization.activeLocale[tostring(str)] or Localization.activeLocale[string.lower(tostring(str))])
          ...
          or (localeTable ~= Localization.localizeNilOnMiss and str or nil)
    end,
  ...
  function Localization:Init()
    ...
    self.activeLocale.ChatMsgIdentifier_LearnedRecipe = self.activeLocale.ChatMsgIdentifier_LearnedRecipe
...
not self.activeLocale[english]
  ...
  function Localization:ReplacePlaceholders(str, locale)
    return (
      string.gsub(str, "(!!(.-)!!)", function(match, placeholderToReplace)
        if self.locales[locale][placeholderToReplace] then
          ...
          Bagshui:PrintDebug(
            "Localization placeholder " .. tostring(match) .. " not found! (from " .. tostring(str) .. ")"
          )
