...
Bagshui:AddComponent(function()
  ...
  function Bagshui:CheckCompat()
    ...
    if pfUI and pfUI.env and pfUI.env.C and pfUI.env.C.disabled then
      ...
      self:CheckOtherAddonSetting("pfUIBags", true, pfUI.env.C.disabled, "bags", "1", "replaceBank")
    end
    ...
    self:CheckOtherAddonSetting(
      "tDFAllInOneBags",
      (_G.IsAddOnLoaded("tDF") and _G.ShaguTweaks and _G.ShaguTweaks.T and _G.ShaguTweaks_config),
  ...
  function Bagshui:CheckOtherAddonSetting(
    ...
    if not addonInstalledCondition or (bagshuiSettingToCheck and not BsSettings[bagshuiSettingToCheck]) then
      return
    end
    ...
    if type(addonSettingTable) == "table" and BsSettings[lastSetting] ~= addonSettingTable[addonSettingKey] then
      BsSettings[ignored] = false
    end

    -- Check actual setting.
    if
      (addonInstalledCondition == DISABLE_ADDON and _G.IsAddOnLoaded(bagshuiCompatSettingId))
      or (
    ...
    then
      ...
      _G.StaticPopupDialogs[dialogName].text = BS_FONT_COLOR.BAGSHUI
        .. "Bagshui"
        .. FONT_COLOR_CODE_CLOSE
        .. BS_NEWLINE
        .. L["Compat_" .. bagshuiCompatSettingId]
