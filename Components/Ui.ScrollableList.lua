...
Bagshui:AddComponent(function()
  ...
  function Ui:CreateScrollableList(params)
    ...
    if not self._scrollableList_SetSelection then
      ...
      self._scrollableItemList_ReceivedItem = function()
        ...
        local targetListFrame = _G.this.bagshuiData.itemDragTargetListFrame
      ...
      self._scrollableList_SetSortOrder = function()
        ...
        if _G.this.bagshuiData.listFrame.bagshuiData.onSortFieldChanged then
          ...
          if
            _G.this.bagshuiData.columnParams.currentSortOrder and not _G.this.bagshuiData.columnParams.lockSortOrder
          then
  ...
  function Ui:PopulateScrollableList(listFrame, entries, refresh, preserveSelection, preserveSearch)
    ...
    if table.getn(listFrame.bagshuiData.entries) > 0 then
      ...
      for i = entryStart, entryEnd, entryStep do
        ...
        local entryFrame =
          self:GetAvailableScrollableListEntryFrame(listFrame, listType, listFrame.bagshuiData.entryFrameCreationFunc)
    ...
    if listType == BS_UI_SCROLLABLE_LIST_TYPE.ITEM then
      ...
      if type(entryInfo) == "table" then
        ...
        if listFrame.bagshuiData.entryColumnTextFunc then
          entryPrimaryText = listFrame.bagshuiData.entryColumnTextFunc(
  ...
  function Ui:ShowScrollableListEntries(listFrame, searchText)
    ...
    for _, entryFrame in ipairs(entryFrames) do
      ...
      if searchText then
        if listType == BS_UI_SCROLLABLE_LIST_TYPE.ITEM then
          -- Item list.
          entryFrame.bagshuiData.show =
            BsRules:Match(searchText, entryFrame.bagshuiData.itemButton.bagshuiData.item, nil, nil, true)
        else
          -- Text list.
          entryFrame.bagshuiData.show = (
            string.find(
              string.lower(BsUtil.RemoveUiEscapes(entryFrame.bagshuiData.textFrames[1]:GetText())),
    ...
    for i, entryFrame in ipairs(entryFrames) do
      if entryFrame.bagshuiData.show then
        ...
        entryFrame:SetPoint(
          ...
          nextAnchorPoint,
          0,
          nextYOffset - ((i > 1 and entryFrame.bagshuiData.isHeader) and HEADER_SPACING or 0)
        )
  ...
  function Ui:UpdateScrollableListEntryFrame(listFrame, entryFrame)
    ...
    if type(ScrollableListCheckbox_OnClick) ~= "function" then
      ...
      function ScrollableListCheckbox_OnClick()
        self:SetScrollableListSelection(_G.this.bagshuiData.listFrame, _G.this.bagshuiData.entryFrame)
      end
    end
  ...
  function Ui:SetScrollableListSelection(listFrame, entryToSelect, selectionState, scrollTo)
    ...
    local toggleSelection = (_G.IsControlKeyDown() or (listFrame.bagshuiData.checkboxes and not _G.IsShiftKeyDown()))
    ...
    if type(entryToSelect) == "table" and entryToSelect.GetParent then
      ...
      if (listFrame.bagshuiData.multiSelect and toggleSelection) or forceSelect or forceDeselect then
      ...
      elseif listFrame.bagshuiData.multiSelect and _G.IsShiftKeyDown() then
        if listFrame.bagshuiData.lastUserSelectionAnchorStart then
          ...
          self:SetScrollableListSelectionRange(
            listFrame,
            entryFrameToSelect.bagshuiData.sequentialFrameNum,
            listFrame.bagshuiData.lastUserSelectionAnchorStart,
            not deselect
          )
      ...
      listFrame.bagshuiData.lastUserSelectionAnchorWasSelected = (
        not entryFrameToSelect.bagshuiData.selected or not _G.IsShiftKeyDown()
      )
    else
    ...
    if entryToSelect and not entryFrameToSelect then
      Bagshui:PrintError(
        "Failed to find selection for entry " .. tostring(entryToSelect) .. " -- this shouldn't happen!"
      )
      return
    end
...
(entryFrame == entryFrameToSelect)
          ...
          and not ((entryFrame == entryFrameToSelect) and listFrame.bagshuiData.multiSelect and entryToSelectWasFrame and entryFrame.bagshuiData.selected and toggleSelection)
          and not deselect
        ) or ((entryFrame == entryFrameToSelect) and forceSelect and not forceDeselect)
      then
      ...
      then
        ...
        for child = listPosition + 1, table.getn(listFrame.bagshuiData.entryFrames) do
          ...
          listFrame.bagshuiData.selectedEntries[listFrame.bagshuiData.entryFrames[child].bagshuiData.scrollableListEntry] =
...
(entryFrame ~= lastHeader and entryFrame.bagshuiData.isHeader and childCount > 0)
    ...
    for _, button in pairs(listFrame.bagshuiData.buttons) do
      ...
      if
        ...
        or (numEntries == 0 and button.bagshuiData.scrollableList_DisableIfEmptyList)
        or (type(button.bagshuiData.scrollableList_DisableFunc) == "function" and button.bagshuiData.scrollableList_DisableFunc(
          listFrame.bagshuiData.selectedEntries
        ))
    ...
    if scrollTo then
      ...
      Bagshui:QueueClassCallback(self, self.ScrollToListEntry, 0.005, false, listFrame, entryFrameToSelect)
    end
  end
  ...
  function Ui:SetScrollableListSelectionRange(listFrame, startNum, endNum, select)
    for i = startNum, endNum, (startNum < endNum and 1 or -1) do
      listFrame.bagshuiData.selectedEntries[listFrame.bagshuiData.entryFrames[i].bagshuiData.scrollableListEntry] = (
        select == true
      ) or nil
    end
  end
